...
def article_switch_vote(redis, user, from_article, to_article):
    # HOMEWORK 2 Part I
    time_aweekago = datetime.datetime.now() - datetime.timedelta(seconds=ONE_WEEK_IN_SECONDS)
    if datetime.datetime.fromtimestamp(redis.zscore('time:',from_article)) >= time_aweekago and \
    datetime.datetime.fromtimestamp(redis.zscore('time:',to_article)) >= time_aweekago:
